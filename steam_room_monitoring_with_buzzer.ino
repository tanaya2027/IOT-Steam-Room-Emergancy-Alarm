...
const float EMERGENCY_TEMP_THRESHOLD = 31.5;  // Temperature threshold in Celsius
...
// Web server details
const char* serverUrl = "http://192.168.168.46/api/temperature";
...
unsigned long buzzerPatternInterval = 1000; // Beep interval in milliseconds
...
void loop() {
  ...
  delay(200);  // Short delay for more responsive buzzer control
}
...
void controlBuzzerPattern() {
  // Create a beeping pattern for the buzzer
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(topic, AWS_TOPIC_SUB) == 0) {
    DynamicJsonDocument doc(256);
    deserializeJson(doc, message);
    
    // Check if this is a command to control the buzzer
    if (doc.containsKey("command")) {
      String command = doc["command"].as<String>();
      
      if (command == "buzzer_on") {
        Serial.println("Remote command: Turning buzzer ON");
        buzzerActive = true;
        buzzerStartTime = millis();
        activateBuzzer();
      } 
      else if (command == "buzzer_off") {
        Serial.println("Remote command: Turning buzzer OFF");
        buzzerActive = false;
        digitalWrite(BUZZER_PIN, LOW);
      }
    }
...
void sendDataToServer(float temperature, float humidity) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    doc["emergency"] = (temperature > EMERGENCY_TEMP_THRESHOLD);
...
void sendEmergencyAlert(float temperature) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    doc["emergency"] = true;
    doc["buzzer_active"] = buzzerActive;
    doc["message"] = "EMERGENCY: Temperature exceeds " + String(EMERGENCY_TEMP_THRESHOLD) + "°C!";
